...
export async function adminListProviders(req: Request, res: Response): Promise<void> {
  const result = await query(
    `SELECT pr.id, pr.partner_id, p.name AS partner_name, p.category AS partner_category,
            pr.full_name, pr.email, pr.role, pr.specialty, pr.mdcn_number, pr.photo_url, pr.phone, pr.is_active, pr.created_at
...
export async function adminCreateProvider(req: Request, res: Response): Promise<void> {
  const { partnerId, fullName, email, password, role = 'doctor', specialty = '', photoUrl = '', phone = '', mdcnNumber = '' } = req.body;
  ...
  const result = await query(
    `INSERT INTO providers (partner_id, full_name, email, password_hash, role, specialty, photo_url, phone, mdcn_number)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
     RETURNING id, partner_id, full_name, email, role, specialty, mdcn_number, photo_url, phone, is_active, created_at`,
    [partnerId, fullName, email, passwordHash, role, String(specialty).slice(0, 120), String(photoUrl).slice(0, 500), String(phone).slice(0, 40), String(mdcnNumber).slice(0, 40)]
  );
...
export async function adminUpdateProvider(req: Request, res: Response): Promise<void> {
  ...
  const result = await query(
    `UPDATE providers
        SET partner_id = $2, full_name = $3, role = $4, specialty = $5, photo_url = $6, is_active = $7, phone = $8, mdcn_number = $9
      WHERE id = $1
      RETURNING id, partner_id, full_name, email, role, specialty, mdcn_number, photo_url, phone, is_active, created_at`,
    [
      id,
      ...
      (b.mdcnNumber ?? b.mdcn_number ?? cur.mdcn_number ?? '').toString().slice(0, 40),
    ]
  );
  await recordAudit(req, {
