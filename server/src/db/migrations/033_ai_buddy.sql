-- Free AI Health Buddy (MVP): grounded, cited, basic health info.
-- Retrieval uses built-in Postgres full-text search over a curated, authoritative
-- corpus (WHO/NHS/MedlinePlus/CDC). pgvector/semantic retrieval is a later upgrade.

-- Curated source passages the buddy may answer from.
CREATE TABLE IF NOT EXISTS buddy_sources (
    ...
    reviewed    BOOLEAN      NOT NULL DEFAULT false,  -- clinician sign-off flag
...
-- Consented conversation log (safety-review queue). session_key is an anonymous
-- client id; no account required for the free buddy.
CREATE TABLE IF NOT EXISTS buddy_messages (
    ...
    safety      VARCHAR(12)  NOT NULL DEFAULT 'ok', -- 'ok' | 'crisis' | 'emergency'
...
-- Free-tier daily cap (e.g. 20 messages/user/day).
...
-- ---- Starter corpus (conservative general info; clinician must review before launch) ----
