-- Daily Health Tips — a free public subscription. Users register a name plus one
-- or more contact channels (SMS, WhatsApp, Email) and receive one tip a day on
-- the channels they chose. Public-health content only, no PHI.
...
CREATE TABLE IF NOT EXISTS health_tip_subscribers (
    ...
    -- Channels the subscriber opted into: any of 'sms','whatsapp','email'.
...
-- The tip library. One is sent per day, rotating through the active tips by seq.
...
-- One row per (subscriber, channel, day) so a day's tip is never sent twice.
...
-- Starter tips so the service works on day one (generic public-health guidance,
-- applies to any insurer/HMO — edit or add more in Admin → Health Tips).
